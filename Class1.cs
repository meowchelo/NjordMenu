...
namespace NjordMenu
{
    ...
    public class NjordMenuGUI : MonoBehaviour
    {
        ...
        public static byte[] spoofMenuRPCs = { 133, 176, 204, 212, 151, 164 };
        ...
        private byte currentColorId = 0;

        // === ПЕРЕМЕННЫЕ SPOOF ===
        public static string spoofLevelString = "100";
        public static string customNameInput = "Njord";
        ...
        public static string[] platformNames = { "Epic", "Steam", "Mac", "Win 10", "Itch", "iOS", "Android", "Switch", "Xbox", "PS", "Unknown" };
        public static Platforms[] platformValues = { Platforms.StandaloneEpicPC, Platforms.StandaloneSteamPC, Platforms.StandaloneMac, Platforms.StandaloneWin10, Platforms.StandaloneItch, Platforms.IPhone, Platforms.Android, Platforms.Switch, Platforms.Xbox, Platforms.Playstation, Platforms.Unknown };
        ...
        public static string fakeStartInput = "99";
        ...
        private void DrawLobbyControls()
        {
            ...
            // НОВАЯ КНОПКА FRAME ALL - запуск корутины
        ...
        public void Update()
        {
            ...
            if (PlayerControl.LocalPlayer != null)
            {
                ...
                // === МУВМЕНТ ТОЛЬКО ДЛЯ NOCLIP ===
                try
                {
                    // Если включен NoClip, двигаемся сквозь стены
                    if (noClip && PlayerControl.LocalPlayer.CanMove)
                    {
                        float moveX = Input.GetAxis("Horizontal") * (invertControls ? -1 : 1);
                        float moveY = Input.GetAxis("Vertical") * (invertControls ? -1 : 1);

                        if (moveX != 0 || moveY != 0)
                        {
                            Vector3 moveDir = new Vector3(moveX, moveY, 0).normalized;
                            PlayerControl.LocalPlayer.transform.position += moveDir * (walkSpeed * 5f * Time.deltaTime);
                        }
                    }
                }
                catch { }

                // === ОБЕСПЕЧЕНИЕ НОУКЛИПА ЧЕРЕЗ ТРИГГЕР ===
                if (PlayerControl.LocalPlayer.Collider != null)
                {
                    PlayerControl.LocalPlayer.Collider.isTrigger = noClip;
                }
            }
            ...
            try
            {
                if (showTracers && PlayerControl.AllPlayerControls != null)
                    foreach (var pc in PlayerControl.AllPlayerControls)
            ...
            if (isEditingName || isEditingLevel || isEditingFakeStart)
            {
                foreach (char c in Input.inputString)
                {
                    ...
                    else if (c == '\n' || c == '\r') { isEditingName = false; isEditingLevel = false; isEditingFakeStart = false; }
                    else
                    {
            ...
            if (unlockFeatures && Time.frameCount % 300 == 0) UnlockCosmetics();
        ...
        public static string GetPlatform(ClientData client)
        {
            ...
            switch ((int)platform)
            {
                case 1: return "Epic Games";
                case 2: return "Steam";
                case 3: return "MacOS";
                case 4: return "Microsoft Store";
                case 5: return "itch.io";
                case 6: return "iOS/iPadOS";
                case 7: return "Android";
                case 8: return "Nintendo Switch";
                case 9: return "Xbox";
        ...
        public static void HandleTracer(PlayerControl target, bool enable)
        {
            try
            {
                ...
                { if (lr != null) lr.enabled = false; return; }
                if (target.Data.IsDead && !seeGhosts && !PlayerControl.LocalPlayer.Data.IsDead)
                { if (lr != null) lr.enabled = false; return; }
                if (lr == null)
                {
                    lr = target.gameObject.AddComponent<LineRenderer>();
                    lr.SetVertexCount(2); lr.SetWidth(0.02f, 0.02f);
                    try { if (HatManager.Instance != null) lr.material = HatManager.Instance.PlayerMaterial; } catch { }
                }
                lr.enabled = true;
                Color tColor = target.Data.IsDead ? Color.gray : (target.Data.Role != null ? GetRoleColor((int)target.Data.Role.Role, target.Data.Role.TeamColor) : Color.white);
    ...
    // ==========================================
    // === ФИКСЫ CHAT: КОМАНДЫ /color, /w И GHOST CHAT ===
    // ==========================================
    ...
    public static class ChatController_SendChat_Patch
    {
        public static bool Prefix(ChatController __instance)
        {
            ...
            // === 1. ЛОКАЛЬНАЯ СМЕНА СВОЕГО ЦВЕТА (/color или /c) ===
            if (lowerChat.StartsWith("/color ") || lowerChat.StartsWith("/c ") || lowerChat.StartsWith("/col "))
            {
                ...
                {
                    HudManager.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "<color=#FF0000>Неверный цвет. Используйте ID (0-18) или названия (red, blue...)</color>");
                }
                __instance.freeChatField.textArea.SetText("", "");
                return false;
            }

            // === 2. КОМАНДА ШЕПОТА (/w или /pm) ===
            if (lowerChat.StartsWith("/w ") || lowerChat.StartsWith("/pm "))
            {
                ...
                if (parts.Length >= 3)
                {
                    ...
                    string[] colorNames = { "red", "blue", "green", "pink", "orange", "yellow", "black", "white", "purple", "brown", "cyan", "lime", "maroon", "rose", "banana", "gray", "tan", "coral", "fortegreen" };

                    if (byte.TryParse(targetInput, out byte pid))
                    {
                        target = PlayerControl.AllPlayerControls.ToArray().FirstOrDefault(p => p.PlayerId == pid);
                    }
                    else if (PlayerControl.AllPlayerControls != null)
                    {
                        foreach (var pc in PlayerControl.AllPlayerControls)
                        {
                            ...
                            string cName = (cId >= 0 && cId < colorNames.Length) ? colorNames[cId] : "";
                            if (rawName == targetInput || rawName.StartsWith(targetInput) || cName == targetInput)
                            {
                                target = pc;
                                break;
                            }
                    ...
                    {
                        HudManager.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "<color=#FF0000>[ОШИБКА]</color> Игрок не найден!");
                    }
                }
                __instance.freeChatField.textArea.SetText("", "");
                return false;
    ...
    public static class ChatController_AddChat_Patch
    {
        public static bool Prefix(PlayerControl sourcePlayer, ref string chatText, ChatController __instance)
        {
            ...
            // === ПУБЛИЧНАЯ КОМАНДА СМЕНЫ ЦВЕТА ОТ ДРУГИХ ИГРОКОВ ===
            if (NjordMenuGUI.enableColorCommand && sourcePlayer != null)
            {
                ...
                if (usedCmd != null)
                {
                    if (AmongUsClient.Instance != null && AmongUsClient.Instance.AmHost)
                    {
                        ...
                        int colorId = -1;
                        if (int.TryParse(colorInput, out int parsedId))
                        {
                            if (parsedId >= 0 && parsedId <= 18) colorId = parsedId;
                        }
                        else colorId = NjordMenuGUI.GetColorIdByName(colorInput);
                        if (colorId != -1) sourcePlayer.RpcSetColor((byte)colorId);
                    }
                    return false; // скрываем команду
                }
            }

            // === GHOST CHAT (отображение сообщений мёртвых) ===
            if (sourcePlayer != null && sourcePlayer.Data != null && PlayerControl.LocalPlayer != null && PlayerControl.LocalPlayer.Data != null)
            {
                bool weAreAlive = !PlayerControl.LocalPlayer.Data.IsDead;
                bool senderIsDead = sourcePlayer.Data.IsDead;

                if (senderIsDead && weAreAlive && NjordMenuGUI.readGhostChat)
                {
                    if (!chatText.StartsWith("<color=#808080>[GHOST]</color>"))
                        chatText = "<color=#808080>[GHOST]</color> " + chatText;
                    ...
                    try
                    {
                        ...
                        pooledBubble.SetCosmetics(sourcePlayer.Data);
                        __instance.SetChatBubbleName(pooledBubble, sourcePlayer.Data, true, didVote, PlayerNameColor.Get(sourcePlayer.Data), null);

                        if (AmongUs.Data.DataManager.Settings.Multiplayer.CensorChat)
                    ...
                    catch (Exception) { __instance.chatBubblePool.Reclaim(pooledBubble); }
                    return false;
                }
            }
            return true;
...
public static class NoClipMove_Patch
{
    public static void Postfix(ref bool __result)
    {
        // Исправлено: NGUI заменено на NjordMenuGUI
        if (NjordMenuGUI.noClip) __result = true;
    }
}

[HarmonyPatch(typeof(PlayerPhysics), nameof(PlayerPhysics.FixedUpdate))]
public static class SafeMovement_Patch
{
    public static void Postfix(PlayerPhysics __instance)
    {
        // Проверяем, что это наш персонаж и сейчас НЕ включен NoClip
        if (__instance.AmOwner && !NjordMenuGUI.noClip && __instance.body != null)
        {
            // Берем чистую скорость, которую только что посчитала сама игра (учитывая стены)
            Vector2 currentVelocity = __instance.body.velocity;

            // Если включена инверсия управления
            if (NjordMenuGUI.invertControls)
            {
                currentVelocity = -currentVelocity;
            }

            // Если включен спидхак, умножаем физическую скорость
            if (NjordMenuGUI.walkSpeed > 1f)
            {
                currentVelocity *= NjordMenuGUI.walkSpeed;

                // Включаем непрерывную коллизию, чтобы на дикой скорости не пролетать сквозь тонкие стены
                __instance.body.collisionDetectionMode = CollisionDetectionMode2D.Continuous;
            }

            // Отдаем просчитанную скорость обратно движку
            __instance.body.velocity = currentVelocity;
        }
    }
}